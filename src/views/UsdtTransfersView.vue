  ...
  <div>
    <a-form layout="inline" :model="searchParams" @submit.prevent="fetchData">
      ...
      <a-form-item label="最少区块交易数">
        <a-input-number v-model="searchParams.minTxsPerBlock" :min="1" placeholder="单区块至少交易数" style="width: 150px;" />
      </a-form-item>
      <a-form-item label="最少Hash交易数">
        <a-input-number v-model="searchParams.minTxsPerHash" :min="1" placeholder="单Hash至少交易数" style="width: 150px;" />
      </a-form-item>
    ...
    <a-space style="margin: 12px 0;">
      ...
      <a-button size="small" @click="applyPreset('blockCluster')">聚集交易 (≥2 txs/block)</a-button>
      <a-button size="small" @click="applyPreset('batchTransfer')">批量转账 (≥3 txs/hash)</a-button>
      <a-button size="small" @click="applyPreset('complexBatch')">复杂批量 (≥5 txs/hash)</a-button>
    </a-space>
    ...
    >
      ...
      <a-form layout="inline" :model="tokenSearchParams" style="margin-bottom: 16px;">
        ...
        <a-form-item label="排序">
          <a-select v-model="tokenSearchParams.sortBy" placeholder="排序字段" style="width: 150px;" @change="fetchTokenList">
            <a-option value="">默认</a-option>
            <a-option value="usdt_count">USDT次数</a-option>
            <a-option value="bnb_amount">BNB金额</a-option>
            <a-option value="created_at">创建时间</a-option>
          </a-select>
        </a-form-item>
        <a-form-item label="排序方式" v-if="tokenSearchParams.sortBy">
          <a-select v-model="tokenSearchParams.order" placeholder="排序方式" style="width: 100px;" @change="fetchTokenList">
            <a-option value="desc">降序</a-option>
            <a-option value="asc">升序</a-option>
          </a-select>
        </a-form-item>
...
const searchParams = ref<{
  ...
  minTxsPerHash: number | undefined
  page: number
  limit: number
  sortBy: string
  order: string
  ...
  minAmount: undefined,
  maxAmount: undefined,
  minTxsPerBlock: undefined,
  minTxsPerHash: undefined,
  page: 1,
  limit: 20,
  sortBy: 'timestamp',
  order: 'desc',
...
const applyPreset = (preset: string) => {
  ...
  switch (preset) {
    ...
    case 'medium':
      ...
      break
    case 'blockCluster':
      searchParams.value.minTxsPerBlock = 2
      Message.info('已应用聚集交易筛选 (≥2 txs/block)')
      break
    case 'batchTransfer':
      searchParams.value.minTxsPerHash = 3
      Message.info('已应用批量转账筛选 (≥3 txs/hash)')
      break
    case 'complexBatch':
      searchParams.value.minTxsPerHash = 5
      Message.info('已应用复杂批量筛选 (≥5 txs/hash)')
      break
  }
  fetchData()
}
...
// Token pagination configuration
...
// Apply address filter (close modal and filter main table by this address)
